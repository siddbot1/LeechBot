...
@bot.on_message(filters.command(["monster_booss"]))
async def account_login(bot: Client, m: Message):
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            url = links[i][1]
            name1 = links[i][0].replace("\t", "").replace(":", "").replace(" https://", ":https://").replace("   ", " - ").replace("/","").replace("+", "").replace("#", "").replace("|", "").replace("@", "").replace("*", "").replace(".", "").strip()
